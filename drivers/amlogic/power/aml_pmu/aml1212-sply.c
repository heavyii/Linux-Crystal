...
int aml_pmu_get_voltage(void)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        tmp = ((((buf[1] & 0x0f) << 8) + buf[0]) * 7200) / 2048;       // LSB of VBAT ADC is 3.51mV
        result += tmp;
    }
    return result / 4;
}
...
static void aml_charger_update_state(struct aml_pmu_charger *charger)
{
    ...
    if (buff[1] & 0x40) {                                               // charge timeout detect
        ...
        if ((charger->charge_timeout_retry) &&
            ...
            aml_pmu_read(AML1212_OTP_GEN_CONTROL0, buff);
            buff[0] &= ~(0x01);
            aml_pmu_write(AML1212_OTP_GEN_CONTROL0, buff[0]);           // close charger
            msleep(1000);
            buff[0] |= (0x01);
            aml_pmu_write(AML1212_OTP_GEN_CONTROL0, buff[0]);           // re open charger
        }
        charge_timeout = 1;
    } else {
        charge_timeout = 0;    
...
static void aml_pmu_charging_monitor(struct work_struct *work)
{
    ...
    if (charger->rest_vol >= 99 && charger->ibat < 150 && charger->bat_current_direction == 1 && !check_charge_flag) {
        ...
        if (!(v[0] & 0x08)) {
            ...
            aml_pmu_read(AML1212_OTP_GEN_CONTROL0, v);
            v[0] &= ~(0x01);
            aml_pmu_write(AML1212_OTP_GEN_CONTROL0, v[0]);              // close charger

            msleep(1000);

            v[0] |= (0x01);
            aml_pmu_write(AML1212_OTP_GEN_CONTROL0, v[0]);              // re open charger
        }
