...
static void kp_search_key(struct kp *kp)
{
	...
	if (key_param[0]) { //virtual key mode channel 0, 1, 2, 3
...
static void circle_move(struct kp *kp)
{
	...
	if(view_count < key_param[20]) {  // if view_count is less than key_param[20]
		x += kp->key_value[1];    // x becomes x + key_value[1]
		y += kp->key_value[0];    // y becomes y + key_value[0]
		kp->key_value[1] = kp->old_x;  // key_value[1] becomes old_x
		kp->key_value[0] = kp->old_y;  // key_value[0] becomes old_y
		view_count++;  // add 1 to view_count every time it runs this
		return;
	} else {  // otherwise
		x /= key_param[20];		 // x becomes x - key_param[20]
		y /= key_param[20];	  // y becomes y - key_param[20]
		view_count = 0;      // view_count becomes 0
	}
	
	if (512 - x >= 0) {  // if it's 512 or higher
		if (x < STEP8)   // and if x is less than 72
			tmp_x = kp->old_x + VALUE8;  //tmp_x becomes old_x + 9
		else if (x < STEP7)  // if x is less than 102
			tmp_x = kp->old_x + VALUE7; // tmp_x becomes old_x + 6
		else if (x < STEP6)  // if x is less than 152
			tmp_x = kp->old_x + VALUE6; // tmp_x becomes old_x + 4
		else if (x < STEP5)  // if x is less than 212
			tmp_x = kp->old_x + VALUE5; // tmp_x becomes old_x + 3
		else if (x < STEP4)  // if x is less than 272
			tmp_x = kp->old_x + VALUE4; // tmp_x becomes old_x + 2
		else if (x < STEP3)  // if x is less than 332
			tmp_x = kp->old_x + VALUE3; // tmp_x becomes old_x + 2
		else if (x < STEP2)  // if x is less than 392
			tmp_x = kp->old_x + VALUE2; // tmp_x becomes old_x + 1
		else if (x < STEP1)  // if x is less than 452
			tmp_x = kp->old_x + VALUE1; // tmp_x becomes old_x + 1
		else
			tmp_x = kp->old_x;  // tmp_x becomes old_x
	} else {
		dx = 1024 - x;  // dx becomes a number between 511 and 0
		if (dx < STEP8)   // and if dx is less than 72
			tmp_x = kp->old_x - VALUE8;  //tmp_x becomes old_x + 9
		else if (dx < STEP7) 
